...
So in the previous section, we were able to extract the version by `read`ing the first 4 bytes and converting that into a u32 integer. However, we want to keep track of where we are in the transaction so that we can continue to do decode it. In order to do that we'll want to pass in the bytes slice to the `extract_version` function and then continue using it in the `main` function. 
...
Notice how we first decode the hex string to get a `Vec` of bytes. We then convert that into a slice and then pass that into the `extract_version` function. We'll now have to modify the `extract_version` function in order to accept the correct argument. 
...
help: consider changing this to be mutable
  |
4 | fn extract_version(mut transaction_bytes: &[u8]) -> u32 {
  |                    +++
```
...
fn main() {
    ...
    println!("bytes_slice: {:?}", bytes_slice); // first 4 bytes [1, 0, 0, 0] should no longer be returned after calling `extract_version`.
